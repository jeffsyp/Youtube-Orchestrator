"""3D Stick People — Comedy: Stick guy tries to sit in a chair. 20 seconds."""
...
STYLE = (
    ...
    "smooth animation, Pixar quality rendering, no text, no watermarks, no UI elements. "
)

CLIPS = [
    {"duration": 4,
     "narration": "All he wanted was to sit down.",
     "prompt": f"{STYLE} A simple 3D rendered white stick figure person walks toward a single red chair in the middle of an empty white room. He reaches for the chair. The chair slides backward away from him on its own. He reaches again. It slides further. He looks confused."},

    {"duration": 8,
     "narration": "The chair had other plans.",
     "prompt": f"{STYLE} A 3D white stick figure person in an empty white room is running after a red chair. The chair slides across the smooth white floor away from him. He dives for it and misses. He lands flat on the floor. The chair stops in the corner. He gets up and tiptoes toward it slowly. He almost reaches it. The chair zooms to the other side of the room. He stands alone in the middle looking frustrated."},

    {"duration": 8,
     "narration": "Fine.",
     "prompt": f"{STYLE} A 3D white stick figure person sits on the bare white floor of an empty room, arms crossed, looking stubborn. Behind him the red chair sits perfectly still. He ignores it. The chair slowly slides closer to him from behind. It nudges against his back gently. He does not react. The chair nudges again. He finally turns around and looks at the chair. He sighs. He sits in the chair. His arms are still crossed. Annoyed but sitting."},
]
...
async def main():
    ...
    try:
        ...
        async with async_session() as session:
            ...
            await session.execute(sql_text("INSERT INTO assets (run_id, channel_id, asset_type, content) VALUES (:rid, 7, :a, :c)"), {"rid": run_id, "a": "publish_metadata", "c": json.dumps({"title": "The Chair That Refused to Be Sat In", "description": "All he wanted was to sit down.\n\n#animation #funny #comedy #3d #stickfigure #Shorts", "tags": ["animation", "funny", "comedy", "3d", "stick figure", "Shorts"], "category": "Entertainment"})})
