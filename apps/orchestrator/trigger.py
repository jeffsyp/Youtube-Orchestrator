"""Trigger a pipeline run. Creates a channel + run in DB, then starts the Temporal workflow."""
...
async def main():
    ...
    async with async_session() as session:
        ...
        if not result.fetchone():
            ...
            logger.info("channel created", channel_id=FAKE_CHANNEL.channel_id)

    # Create a content run
    async with async_session() as session:
        result = await session.execute(
            text("INSERT INTO content_runs (channel_id, status) VALUES (:cid, 'running') RETURNING id"),
            {"cid": FAKE_CHANNEL.channel_id},
        )
        run_id = result.scalar_one()
        await session.commit()
        logger.info("content run created", run_id=run_id)
    ...
    handle = await client.start_workflow(
        ...
        id=f"daily-pipeline-run-{run_id}",
        task_queue="daily-content-pipeline",
    )

    logger.info("workflow started", workflow_id=handle.id, run_id=run_id)
    ...
    logger.info("run marked complete", run_id=run_id)
