...
async def prescreen_satisdefying_clips(run_id: int, channel_id: int, clips: list[str], concept: dict) -> list[dict]:
    ...
    for i, clip_path in enumerate(clips):
        ...
        review_prompt = f"""Watch this AI-generated ASMR/satisfying video clip. This is clip {i+1} of {len(clips)}.

INTENDED: {prompt_text}

Score 1-10 on:
1. Does it match the intended satisfying content?
2. Visual quality — macro detail, lighting, textures?
3. Audio quality — are there satisfying ASMR sounds? Crunching, slicing, squelching?
