...
async def pick_satisdefying_concepts(run_id: int, channel_id: int) -> list[dict]:
    ...
    if USE_REAL_WRITING:
        ...
        from packages.prompts.satisdefying import build_satisdefying_ideas_prompt, _SYSTEM as SATIS_RULES
...
async def generate_satisdefying_clips(run_id: int, channel_id: int, concept: dict) -> list[str]:
    ...
    if USE_SORA:
        ...
        clip_durations = concept.get("clip_durations", [channel_config.get("sora_duration", 10)] * len(sora_prompts))
...
async def prescreen_satisdefying_clips(run_id: int, channel_id: int, clips: list[str], concept: dict) -> list[dict]:
    ...
    for i, clip_path in enumerate(clips):
        ...
        review_prompt = build_review_prompt(concept, "Satisdefying", "AI-generated ASMR satisfying videos", video_feedback=video_fb or None)
