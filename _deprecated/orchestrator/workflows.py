...
class DailyContentPipeline:
    ...
    async def run(self, run_id: int, channel_id: int) -> dict:
        ...
        # 17. Mark as pending human review instead of auto-publishing
        publish_metadata = {
            "title": package.get("title", ""),
            "description": package.get("description", ""),
            "tags": package.get("tags", []),
            "category": package.get("category", "Science & Technology"),
            "thumbnail_path": thumbnail.get("path") if thumbnail and thumbnail.get("status") == "generated" else None,
        }
        await workflow.execute_activity(
