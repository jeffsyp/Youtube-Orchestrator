...
render → QA → video review (Gemini) → pending_review (no post-render gates)
"""
...
class ShortsPipeline:
    ...
    async def select_concept(self, index: int) -> None:
        """Alias so the concept queue can use a unified signal name."""
        self._selected_topic_index = index
    ...
    async def run(self, run_id: int, channel_id: int, auto_pick: bool = False,
        ...
        if auto_pick:
            ...
            await workflow.wait_condition(
                lambda: self._selected_topic_index is not None or self._rejected)
            if self._rejected:
                return {"status": "rejected", "reason": "All topics rejected by user"}
        ...
        # 10. Mark as pending human review instead of auto-publishing
