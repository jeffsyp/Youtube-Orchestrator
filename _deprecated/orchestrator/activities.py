...
@activity.defn
async def store_pending_concepts(run_id: int, channel_id: int, concepts: list[dict]) -> None:
    """Store all qualifying concepts as a pending_concepts asset for the review queue."""
    await _execute(
           ...
           VALUES (:run_id, :channel_id, :type, :content)""",
        {
            ...
            "type": "pending_concepts",
            "content": json.dumps(concepts),
        },
    )
    log = logger.bind(activity="store_pending_concepts", run_id=run_id)
    log.info("pending concepts stored", count=len(concepts))


@activity.defn
async def mark_run_pending_review(run_id: int, publish_metadata: dict | None = None) -> dict:
    """Mark a content run as pending human review.

    Args:
        run_id: The content run ID.
        publish_metadata: Optional dict with title, description, tags, category
            to store for later publishing from the review queue.
    """
    await _execute(
        "UPDATE content_runs SET status = 'pending_review', current_step = 'awaiting_human_review', completed_at = NOW() WHERE id = :run_id",
        {"run_id": run_id},
    )

    if publish_metadata:
        # Store publish metadata so the review queue can upload without
        # needing the original workflow concept dict
        async with async_session() as session:
            result = await session.execute(
                text("SELECT channel_id FROM content_runs WHERE id = :run_id"),
                {"run_id": run_id},
            )
            row = result.fetchone()
        ...
        await _execute(
               ...
               VALUES (:run_id, :channel_id, :type, :content)""",
            {
                ...
                "content": json.dumps(publish_metadata),
            },
        )

    log = logger.bind(activity="mark_run_pending_review", run_id=run_id)
    log.info("run pending review")
    return {"run_id": run_id, "status": "pending_review"}
