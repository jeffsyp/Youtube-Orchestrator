"""Temporal activity implementations for the Gamatatsuken pipeline.
    ...
    async with async_session() as session:
        result = await session.execute(
            text("""SELECT DISTINCT s.idea_title FROM scripts s
                    ...
                    WHERE s.channel_id = :cid AND cr.content_type = 'gamatatsuken'
...
@activity.defn
async def pick_gamatatsuken_concepts(run_id: int, channel_id: int) -> list[dict]:
    """Generate narrated anime short film concept ideas."""
    await _update_run_step(run_id, "pick_concepts")
    log = logger.bind(activity="pick_gamatatsuken_concepts", run_id=run_id)

    if USE_REAL_WRITING:
        from apps.orchestrator.concept_engine import generate_idea_pitches
        from packages.prompts.gamatatsuken import build_gamatatsuken_series_ideas_prompt as build_gamatatsuken_ideas_prompt, _SERIES_SYSTEM as ANIME_RULES

        channel_config = await _get_channel_config_raw(channel_id)
        concepts, insights = await generate_idea_pitches(
            channel_id=channel_id,
            channel_name=channel_config.get("_channel_name", "Gamatatsuken"),
            channel_niche=channel_config.get("_channel_niche", "Narrated anime short films"),
            content_type="gamatatsuken",
            ideas_prompt_builder=build_gamatatsuken_ideas_prompt,
            channel_rules=ANIME_RULES,
        )
        if insights:
        ...
        concepts = [
            {
                ...
                "sora_prompts": [
                    "Anime animation style, bold dramatic lines, vibrant colors, dynamic speed lines, dramatic lighting with lens flares. A young boy with messy black hair and glowing golden eyes stands in a massive crater. Dozens of warriors lie defeated around him. Golden energy radiates from his body.",
                    "Anime animation style, bold dramatic lines, vibrant colors, dramatic lighting. A small thin boy sits alone on a bench outside a training academy. Other students walk past laughing. He stares at his hands. Rain falls.",
                    "Anime animation style, bold dramatic lines, dynamic speed lines. The boy trains alone in a forest at night. He throws weak energy blasts at trees. Falls. Gets up. Tries again. His hands start to glow faintly gold.",
                    "Anime animation style, bold dramatic lines, vibrant colors, dramatic lighting with lens flares. The boy faces the top student in a tournament. Golden light erupts from his palms. He catches a red energy blast and pushes it back.",
                    "Anime animation style, bold dramatic lines, vibrant colors, dynamic speed lines, dramatic lighting with lens flares. The boy unleashes full golden power. A pillar of light shoots into the sky. He stands in the crater victorious.",
                ],
                "narration": [
                    "They said he would never amount to anything. They were wrong.",
                    "He was born without a trace of power. The weakest student in the academy.",
                    "But every night, when no one was watching, he trained. He fell. He got back up.",
                    "And then it happened. The power that was never supposed to exist inside him erupted.",
                    "The boy they called Nothing became the strongest of them all.",
                ],
                "caption": "They called him nothing",
                "description": "The weakest student becomes the strongest warrior. #anime #shorts #Shorts",
                "tags": ["anime", "shorts", "underdog", "power", "Shorts"],
                "score": 9.0,
            },
        ]
...
@activity.defn
async def store_gamatatsuken_concept(run_id: int, channel_id: int, concept: dict) -> None:
...
@activity.defn
async def generate_gamatatsuken_clips(run_id: int, channel_id: int, concept: dict) -> list[str]:
    """Generate Sora video clips for a narrated anime concept."""
    await _update_run_step(run_id, "generate_clips")
    log = logger.bind(activity="generate_gamatatsuken_clips", run_id=run_id, title=concept.get("title"))

    output_dir = f"output/gamatatsuken_run_{run_id}/clips"
    ...
    if USE_SORA:
        ...
        from packages.prompts.gamatatsuken import refine_sora_prompt
        prev_clip_ref = None
        ...
        for i, _ in enumerate(sora_prompts):
            ...
            result = await generate_video_async(
                ...
                duration=clip_durations[i] if i < len(clip_durations) else 12,
                size=sora_size,
                timeout=1200,
...
@activity.defn
async def prescreen_gamatatsuken_clips(run_id: int, channel_id: int, clips: list[str], concept: dict) -> list[dict]:
    """Use Gemini to watch each clip and check quality + prompt match."""
    await _update_run_step(run_id, "prescreen_clips")
    log = logger.bind(activity="prescreen_gamatatsuken_clips", run_id=run_id)
    ...
    for i, clip_path in enumerate(clips):
        ...
        review_prompt = build_review_prompt(concept, "Gamatatsuken", "Narrated anime short films", video_feedback=video_fb or None)
...
@activity.defn
async def render_gamatatsuken_short(run_id: int, channel_id: int, clips: list[str], concept: dict) -> dict:
    """Render the final Gamatatsuken Short with narration.
    ...
    log = logger.bind(activity="render_gamatatsuken_short", run_id=run_id)
    ...
    concat_path = os.path.join(output_dir, "gamatatsuken_concat.mp4")
    ...
    final_path = os.path.join(output_dir, "gamatatsuken_short.mp4")
    ...
    try:
        ...
        thumbnail_path = generate_shorts_thumbnail(
            ...
            title=concept.get("caption", concept.get("title", "Gamatatsuken")),
    ...
    result = {
        ...
        "resolution": "720x1280",
        "content_type": "gamatatsuken_short",
    ...
    await _execute(
        ...
        {
            "run_id": run_id, "channel_id": channel_id,
            "type": "rendered_gamatatsuken_short", "content": json.dumps(result),
        },
    )

    log.info("gamatatsuken short rendered", path=result.get("path"),
             duration=round(final_duration), narration_clips=result["narration_clips"])
    return result


@activity.defn
async def gamatatsuken_qa_check(run_id: int, channel_id: int, rendered: dict) -> dict:
    """QA check for a Gamatatsuken Short."""
    await _update_run_step(run_id, "qa_check")
    log = logger.bind(activity="gamatatsuken_qa_check", run_id=run_id)
    ...
    # Duration should be ~60s (5 clips x 12s minus crossfade overlap)
    ...
    if all_passed:
        log.info("gamatatsuken QA passed")
    else:
        log.warning("gamatatsuken QA FAILED", issues=issues)
    return report


@activity.defn
async def review_gamatatsuken_video(run_id: int, channel_id: int, rendered: dict, concept: dict) -> dict:
    """Use Gemini to watch and critique the final video."""
    await _update_run_step(run_id, "video_review")
    log = logger.bind(activity="review_gamatatsuken_video", run_id=run_id)
    ...
    prompt = f"""Watch this narrated anime short film YouTube Short and critique it.
...
Score 1-10 on:
1. STORY: Is there a clear narrative arc? Does the hook-rewind-escalation-payoff structure work?
2. NARRATION: Does the voice narration enhance the story? Is the timing good? Does it match the visuals?
3. VISUAL QUALITY: Does it look like anime? Bold lines, vibrant colors, dynamic action, lens flares?
4. CONTINUITY: Do the clips feel like one continuous story? Same characters, same style throughout?
5. HOOK: Does clip 1 grab attention instantly with a dramatic climax preview?

Return JSON (no markdown):
{{"story_score": 8, "narration_score": 7, "visual_score": 8, "continuity_score": 7, "hook_score": 7, "overall_score": 7.4, "publish_recommendation": "yes/no/maybe", "top_issue": "Biggest problem", "summary": "One sentence verdict", "suggestions": ["Improvement 1", "Improvement 2"], "reviewed": true}}"""
...
@activity.defn
async def publish_gamatatsuken_short(run_id: int, channel_id: int, concept: dict, qa: dict, rendered: dict) -> dict:
    """Upload Gamatatsuken Short to YouTube."""
    await _update_run_step(run_id, "publish")
    log = logger.bind(activity="publish_gamatatsuken_short", run_id=run_id)
