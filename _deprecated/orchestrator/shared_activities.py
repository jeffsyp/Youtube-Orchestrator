...
async def generate_concept_detail(
    ...
    from apps.orchestrator.concept_engine import generate_detailed_prompts, get_video_feedback
    ...
    # Pull user video feedback
    video_fb = await get_video_feedback(channel_id)
    if video_fb:
        video_fb_block = "\n\nUSER VIDEO FEEDBACK (MUST FOLLOW — these are direct instructions about how the rendered video should look/feel):\n"
        video_fb_block += "\n".join(f"- {fb}" for fb in video_fb)
        feedback = (feedback + video_fb_block) if feedback else video_fb_block
        log.info("injecting video feedback into prompts", count=len(video_fb))
...
async def retry_failed_clips(
    ...
    # Post-Sora: clips are already generated (paid for), never reject them.
    # Prescreen data is still collected and stored for display, but no clip
    # is ever re-generated based on score.  The user decides in the review queue.
    failed = []
    if not failed:
