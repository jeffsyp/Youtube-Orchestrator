...
async def pick_lad_stories_concepts(run_id: int, channel_id: int) -> list[dict]:
    ...
    if USE_REAL_WRITING:
        ...
        from packages.prompts.lad_stories import build_lad_stories_ideas_prompt, _SYSTEM as LAD_RULES
...
async def prescreen_lad_stories_clips(run_id: int, channel_id: int, clips: list[str], concept: dict) -> list[dict]:
    ...
    for i, clip_path in enumerate(clips):
        ...
        review_prompt = build_review_prompt(concept, "Lad Stories", "Claymation character adventures", video_feedback=video_fb or None)
