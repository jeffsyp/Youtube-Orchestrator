...
async def pick_synthzoo_concepts(run_id: int, channel_id: int) -> list[dict]:
    ...
    if USE_REAL_WRITING:
        ...
        from packages.prompts.synthzoo import build_synthzoo_ideas_prompt, _SYSTEM as SYNTHZOO_RULES
    ...
    if strong:
        ...
        concepts = concepts[:1]  # keep at least one to avoid crash
...
Set "passed" to false if match_score < 5 or quality_score < 5, or if clip 1 has hook_score < 5. Be permissive — clips are already generated so we should use them unless they are genuinely broken or wrong."""
...
async def review_synthzoo_video(run_id: int, channel_id: int, rendered: dict, concept: dict) -> dict:
    ...
    prompt = build_review_prompt(concept, "Synth Meow", "AI-generated animal videos", video_feedback=video_fb or None)
