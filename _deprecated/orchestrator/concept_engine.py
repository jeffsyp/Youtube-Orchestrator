...
async def get_concept_feedback(channel_id: int) -> list[str]:
    """Fetch user concept feedback for a channel."""
    async with async_session() as session:
        result = await session.execute(
        ...
        for row in result.fetchall():
            try:
                ...
                if data.get("feedback"):
                    feedback_items.append(data["feedback"])
            except (json.JSONDecodeError, TypeError):
                continue
        return feedback_items


async def get_video_feedback(channel_id: int) -> list[str]:
    """Fetch user video feedback for a channel."""
    async with async_session() as session:
        result = await session.execute(
            text("""SELECT content FROM assets
...
async def generate_idea_pitches(
    ...
    # Inject user concept feedback into channel rules
    concept_feedback = await get_concept_feedback(channel_id)
    if concept_feedback:
        feedback_block = "\n\nUSER FEEDBACK FOR THIS CHANNEL (MUST FOLLOW):\n"
        feedback_block += "\n".join(f"- {fb}" for fb in concept_feedback)
        channel_rules = channel_rules + feedback_block
        log.info("injecting concept feedback", count=len(concept_feedback))
